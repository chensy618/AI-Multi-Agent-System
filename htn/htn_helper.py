...
class HTNHelper:
    ...
    def prioritize_goal_box_by_difficulty(available_goals, boxes, agent):
        ...
        sorted_goals = sorted(available_goals, key=lambda available_goals: (-available_goals.x2, -available_goals.y2, -available_goals.x1, -available_goals.y1, -available_goals.z, -available_goals.group,))
        
        # for goal in sorted_goals:
            # print(f"uid: {goal.uid}, value:{goal.value}, x1:{goal.x1}, y1:{goal.y1}, x2:{goal.x2}, y2:{goal.y2}, z:{goal.z}, g:{goal.group}", file=sys.stderr)
